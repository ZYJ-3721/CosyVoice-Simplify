...
from cosyvoice.utils.common import fade_in_out, TrtContextWrapper
from cosyvoice.utils.file_utils import convert_onnx_to_trt, export_cosyvoice2_vllm


class CosyVoiceModel:
    def __init__(self, llm: torch.nn.Module, flow: torch.nn.Module, hift: torch.nn.Module, fp16: bool = False,
    ...
    def load(self, llm_model, flow_model, hift_model):
        ...
        self.flow.load_state_dict(
            state_dict=torch.load(flow_model, map_location=self.device))
        self.flow.to(self.device).eval()
        self.hift.load_state_dict(
            state_dict={k.replace("generator.", ""): v for k, v in torch.load(hift_model, map_location=self.device).items()})
    ...
    def load_trt(self, flow_decoder_estimator_model, flow_decoder_onnx_model, trt_concurrent, fp16):
        ...
        if not os.path.exists(flow_decoder_estimator_model) or os.path.getsize(flow_decoder_estimator_model) == 0:
            convert_onnx_to_trt(
                fp16=fp16,
                onnx_model=flow_decoder_onnx_model,
                trt_model=flow_decoder_estimator_model,
                trt_kwargs={
        ...
        assert estimator_engine is not None, f"failed to load TensorRT engine from {flow_decoder_estimator_model}"
        self.flow.decoder.estimator = TrtContextWrapper(estimator_engine, trt_concurrent=trt_concurrent, device=self.device)
    
    def llm_job(self, text, prompt_text, llm_prompt_speech_token, llm_embedding, uuid):
        with self.llm_context, torch.amp.autocast("cuda", enabled=self.fp16 and not hasattr(self.llm, "vllm")):
            if isinstance(text, Generator):
                ...
                for i in self.llm.inference_bistream(
                    ...
                    prompt_speech_token_len=torch.tensor([llm_prompt_speech_token.shape[1]], dtype=torch.int32, device=self.device),
    ...
    def token2wav(self, token, prompt_token, prompt_feat, embedding, uuid, finalize=False, speed=1.0):
        ...
        if self.hift_cache_dict[uuid] is None:
            ...
            hift_cache_source = self.hift_cache_dict[uuid]["source"]
    ...
    def tts(self, text=torch.zeros(1, 0, dtype=torch.int32),
            ...
            flow_embedding=torch.zeros(0, 192, dtype=torch.float32), 
            llm_embedding=torch.zeros(0, 192, dtype=torch.float32),
        ...
        if stream is True:
            ...
            while True:
                ...
                if len(self.tts_speech_token_dict[this_uuid]) >= token_hop_len + self.token_overlap_len:
                    ...
                    this_tts_speech = self.token2wav(
                        token=this_tts_speech_token, prompt_token=flow_prompt_speech_token,
                        prompt_feat=prompt_speech_feat, embedding=flow_embedding,
                        uuid=this_uuid, finalize=False, speed=1)
            ...
            this_tts_speech = self.token2wav(
                token=this_tts_speech_token, prompt_token=flow_prompt_speech_token,
                prompt_feat=prompt_speech_feat, embedding=flow_embedding,
                uuid=this_uuid, finalize=True, speed=1)
            yield {"tts_speech": this_tts_speech.cpu()}
        else:
            p.join()
            ...
            this_tts_speech = self.token2wav(
                token=this_tts_speech_token, prompt_token=flow_prompt_speech_token,
                prompt_feat=prompt_speech_feat, embedding=flow_embedding,
                uuid=this_uuid, finalize=True, speed=speed)
            yield {"tts_speech": this_tts_speech.cpu()}
        with self.lock:
...
class CosyVoice2Model(CosyVoiceModel):
    ...
    def load_vllm(self, model_dir, gpu_memory_utilization=0.2):
        from vllm import ModelRegistry, EngineArgs, LLMEngine
    ...
    def token2wav(self, token, prompt_token, prompt_feat, embedding, token_offset, uuid, finalize=False, speed=1.0, stream=False):
        with torch.amp.autocast("cuda", enabled=self.fp16):
    ...
    def tts(self, text=torch.zeros(1, 0, dtype=torch.int32),
        ...
        if stream is True:
            ...
            while True:
                ...
                if len(self.tts_speech_token_dict[this_uuid]) - token_offset >= this_token_hop_len + self.flow.pre_lookahead_len:
                    ...
                    this_tts_speech = self.token2wav(
                        token=this_tts_speech_token, prompt_token=flow_prompt_speech_token,
                        prompt_feat=prompt_speech_feat, embedding=flow_embedding,
                        token_offset=token_offset, stream=stream,
                        uuid=this_uuid, finalize=False, speed=1)
            ...
            this_tts_speech = self.token2wav(
                token=this_tts_speech_token, prompt_token=flow_prompt_speech_token,
                prompt_feat=prompt_speech_feat, embedding=flow_embedding,
                token_offset=token_offset, stream=False,
                uuid=this_uuid, finalize=True, speed=1)
            yield {"tts_speech": this_tts_speech.cpu()}
        else:
            p.join()
